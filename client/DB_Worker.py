...
class MDOos:
    def integrityCheck(self) -> bool:
        ...
        if not os.path.exists(self.file1):
            self.console_instance.consoleSend(
                "#CD3333", f"FATAL ERROR: No {self.file1}"
            )
        ...
        if not os.path.exists(self.file2):
            self.console_instance.consoleSend(
                "#CD3333", f"FATAL ERROR: No {self.file2}"
            )
    ...
    def searchIssuesCSV(
        self, pd_Name_Column, cl_Name_Column, cl_SKU_Column, pd_SKU_Column
    ) -> list:
        self.console_instance.consoleSend(
            "#DFFF00", f"Starting Main worker looking for UPC"
        )
        fixQueue = []
        self.counter = 0
        self.found = 0
                            ...
                            for lines in self.csv_reader_DB:
                                ...
                                if lines[cl_Name_Column] == row[pd_Name_Column]:
                                    ...
                                    try:
                                        self.console_instance.consoleSend(
                                            "green",
                                            f"UPC: {row[pd_Name_Column]} | {lines[cl_SKU_Column]}",
                                        )
                                        print(
                                            f"Found UPC for UPC: {row[pd_Name_Column]} | ({lines[cl_SKU_Column]})"
                                        )
                                        self.counter = self.counter - 1
    ...
    def fixDB(self, fixList, date, pd_SKU_Column, outputName, data) -> bool:
        ...
        for item in fixList:  # LINE = item
            ...
            if True:
                ...
                dataF[item[2]][pd_SKU_Column] = str(
                    int(item[1])
        ...
        with open((outputName + f"-{date}.csv"), "w", newline="") as file1:
            ...
            if bool(data["fixIssuesNumbers"]) == True:
                ...
                for row in dataF:
                    if len(row) > 1:
                        for i in range(len(row)):  # Iterate over each column in the row
                            if row[i].endswith(
                                ".0"
                            ):  # Check if the value in the column ends with ".0"
                                row[i] = row[i][:-2]  # Remove the last two characters
    ...
    def startWorker(self):
        ...
        if not self.integrityCheck():
            ...
            return False
        print("Les goooooooo")
        fixingList = self.searchIssuesCSV(
            pd_Name_Column, cl_Name_Column, cl_SKU_Column, pd_SKU_Column
        )
        ...
        if self.fixDB(
            ...
            self.console_instance.consoleSend(
                "green", f"-- Worker offline (task completed!)--"
            )
            return True
