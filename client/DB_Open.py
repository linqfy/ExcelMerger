...
from Util.GUI_tableSelector import tooltipSelector
import json
...
class Reader:
    def __init__(self, window, console):
        self.window = window
        self.tooltip = tooltipSelector(
            window
        )  # Instantiate the tooltipSelector class correctly
    ...
    def read_file(self, file_path, type):
        ...
        try:
            ...
            if type == "prod":
                ...
                self.console_instance.consoleSend(
                    "yellow",
                    f"Reading prod db.. ({file_path}, on table: {pd_Num_Table})",
                )

                df = pd.read_excel(dm, sheet_name=pd_Num_Table)  # , encoding=encoding)
                active_sheet_name = dm.sheet_names[
                    pd_Num_Table
                ...
                try:
                    selectedSheet = self.tooltip.gui_select(dfs)
                    print("Selected value:", selectedSheet)
                except Exception as e:
                    print(e)
                self.console_instance.consoleSend(
                    "yellow",
                    f"Reading client db.. ({file_path}, on table: {selectedSheet})",
                )
                df = pd.read_excel(dm, sheet_name=selectedSheet)  # , encoding=encoding)
                print("Active sheet name:", selectedSheet)
            ...
            if len(dfs) > 1:
                ...
                self.console_instance.consoleSend(
                    "yellow",
                    f"WARNING: Table has multiple sheets {dfs}, selecting default on config.json",
                )
            else:
                print("The Excel file has only one sheet.")
                self.console_instance.consoleSend(
                    "green", f"Clean! No other tables were detected"
                )
            if active_sheet_name is not None:
                return [df, active_sheet_name]
            return [df, selectedSheet]
        except Exception as e:
    ...
    def ReadStartProd(self, ProdDB):
        try:
            ...
            if encoding:
                ...
                self.console_instance.consoleSend(
                    "#CD3333", f"Error reading prod DB: Unable to determine encoding."
                )
        except Exception as e:
            print(f"Reading error on db prod... {e}")
            self.console_instance.consoleSend(
                "#CD3333", f"ERROR on prod DB issued at: {e}"
            )
        return ProdDB
    ...
    def ReadStartClient(self, client_db):
        try:
            ...
            if encoding:
                ...
                self.console_instance.consoleSend(
                    "#CD3333", f"Error reading client DB: Unable to determine encoding."
                )
        except Exception as e:
            print(f"Reading error on db client... {e}")
            self.console_instance.consoleSend(
                "#CD3333", f"ERROR on client DB issued at: {e}"
            )
        return client_db
