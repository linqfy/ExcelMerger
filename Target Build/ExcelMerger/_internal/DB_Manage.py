...
class DatabaseConnectionTool():
    ...
    def UpdateProd(self, fileProd):
        ...
        if self.fileProd is not None:
            ...
            self.fileProd.to_csv(os.path.join(dirCurrent, "cache", "cachePROD.csv"), index=False)
    ...
    def UpdateClient(self, fileClient):
        ...
        if self.fileClient is not None:
            # Write the DataFrame to a CSV file
            self.fileClient.to_csv(os.path.join(dirCurrent, "cache", "cacheCLIENT.csv"), index=False)